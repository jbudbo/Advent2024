...
{
    [GeneratedRegex("mul\\((\\d{1,3}),(\\d{1,3})\\)", RegexOptions.IgnoreCase | RegexOptions.NonBacktracking)]
    private static partial Regex GetPart1Regex();

    [GeneratedRegex("mul\\((\\d{1,3}),(\\d{1,3})\\)|do\\(\\)|don't\\(\\)", RegexOptions.IgnoreCase | RegexOptions.NonBacktracking)]
